...
async function main() {
    ...
    await prisma.medicalHistory.deleteMany().catch(() => { });
    ...
    await prisma.patientReminder.deleteMany().catch(() => { });
    await prisma.appointmentCancellation.deleteMany().catch(() => { });
    await prisma.orgPatientSnapshot.deleteMany().catch(() => { });
    ...
    const doctor1 = await prisma.user.upsert({
        where: { email: 'sarah@cityhospital.com' },
    ...
    const doctor2 = await prisma.user.upsert({
        where: { email: 'michael@cityhospital.com' },
        update: { password: docPassword },
        create: {
    ...
    const receptionist = await prisma.user.upsert({
        where: { email: 'mary@cityhospital.com' },
